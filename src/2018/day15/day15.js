...
const { isEmpty, without, minBy, flatMap, padStart, sortBy } = _;
...
const findInRangeTarget = (unit, targets) => {
  ...
  return minBy(inRange, (t) => `${padStart(t.hp, 3, 0)},${padStart(t.y, 3, 0)},${padStart(t.x, 3, 0)}`);
};
...
const emptyAdjacent = (game, x, y) => {
  ...
  return candidates.filter(([cx, cy]) => curGrid[cy][cx] === '.').map(([cx, cy]) => ({ x: cx, y: cy }));
};
